<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          v-hasPermi="['system:app-client-menu:create']"
          plain
          type="primary"
  ...
  <!-- 表单弹窗：添加/修改 -->
  <AppClientMenuForm ref="formRef" @success="getList" />
</template>
<script lang="tsx" setup>
import { DICT_TYPE, getIntDictOptions } from '@/utils/dict'
import { handleTree } from '@/utils/tree'
import * as AppClientMenuApi from '@/api/system/appClientMenu'
import { AppClientMenuVO } from '@/api/system/appClientMenu'
import AppClientMenuForm from './AppClientMenuForm.vue'
...
defineOptions({ name: 'AppClientMenu' })
...
const columns = [
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }) => {
      // 检查权限
      if (!checkPermi(['system:app-client-menu:update'])) {
  ...
  },
  {
    ...
    cellRenderer: ({ rowData }) => {
      ...
      if (checkPermi(['system:app-client-menu:create'])) {
        buttons.push(
          <ElButton
            key="create"
            link
      ...
      if (checkPermi(['system:app-client-menu:delete'])) {
        buttons.push(
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await AppClientMenuApi.deleteAppClientMenu(id)
...
const handleStatusChanged = async (menu: AppClientMenuVO, val: number) => {
  // 1. 标记 menu.id 更新中
  menuStatusUpdating.value[menu.id] = true
  try {
    // 2. 发起更新状态
    menu.status = val
    await AppClientMenuApi.updateAppClientMenu(menu)
  } finally {
    // 3. 标记 menu.id 更新完成
