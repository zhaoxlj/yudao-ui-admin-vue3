...
defineOptions({ name: 'AppClientRoleForm' })
...
const open = async (type: string, id?: number) => {
  ...
  dialogTitle.value = type === 'create' ? '新增客户端角色' : '修改客户端角色'
  formType.value = type
  resetForm()
  // 修改时，设置数据
  if (id) {
    ...
    try {
      // TODO: 后端需要添加get接口
      // formData.value = await AppClientRoleApi.getAppClientRole(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as AppClientRoleApi.AppClientRoleVO
    if (formType.value === 'create') {
      await AppClientRoleApi.createAppClientRole(data)
      message.success(t('common.createSuccess'))
    } else {
      await AppClientRoleApi.updateAppClientRole(data)
