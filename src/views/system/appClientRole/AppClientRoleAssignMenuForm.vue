...
defineOptions({ name: 'AppClientRoleAssignMenuForm' })
...
/** 打开弹窗 */
const open = async (row: AppClientRoleApi.AppClientRoleVO) => {
  dialogVisible.value = true
  resetForm()
  // 加载 Menu 列表。注意，必须放在前面，不然下面 setChecked 没数据节点
  menuOptions.value = handleTree(await AppClientMenuApi.getAppClientSimpleMenuList())
  // 设置数据
  formData.id = row.id
  ...
  try {
    formData.menuIds = await AppClientRoleApi.getRoleMenus(row.id)
    // 设置选中
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    await AppClientRoleApi.assignRoleMenu(formData.id, menuIds)
